...
public class SSLTestClientBalance {
	...
	public static void main(String[] args) {
		...
		configuration.setSelectorStrategy(EnumSelectorConnectorStrategy.BalanceConnector);
		...
		try {
			...
			client = new SSLSmartClient("client", configuration, DecoderTest.class, EncoderTest.class, new EventFactoryBalance());

			logger = new SmartLog(client);

			logger.info("Teste de client no modo balanceado");
			...
			outMessage = new MessageTest();
			outMessage.setText("Mensagem 4 de teste");
			...
			logger.error("Falha durante teste de cliente no modo balanceado. Erro: %s", ExceptionUtils.rootCauseMessage(e));;
		}
